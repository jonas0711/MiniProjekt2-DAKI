...
def extract_features_rgb_texture(images):
    """
    Udtrækker RGB og tekstur features fra billeder.
    """
    features = []
    
    for i, image in enumerate(images):
        if i % 100 == 0:
            print(f"Udtrækker RGB+Tekstur features for billede {i+1}/{len(images)}...")
        ...
        # Kombiner features
        combined_features = np.concatenate([rgb_hist, texture_hist])
        
        features.append(combined_features)
    
    return np.array(features)

def extract_features_hsv_texture(images):
    """
    Udtrækker HSV og tekstur features fra billeder.
    """
    features = []
    
    for i, image in enumerate(images):
        if i % 100 == 0:
            print(f"Udtrækker HSV+Tekstur features for billede {i+1}/{len(images)}...")
...
def evaluate_model(model, X_test, y_test, terrain_classes, filename_suffix=""):
    """
    ...
    plt.savefig(f'confusion_matrix_{filename_suffix}.png')
    
    print(f"Resultater gemt til confusion_matrix_{filename_suffix}.png")
    
    # Returner nøjagtighed til reference
    return np.mean(y_pred == y_test)

def run_experiment(experiment_name, feature_extraction_func):
    """
    Kører et eksperiment med en specifik feature-extraction funktion.
    """
    print(f"\n{'='*50}")
    print(f"Starter eksperiment: {experiment_name}")
    print(f"{'='*50}\n")
    ...
    X_train_features = feature_extraction_func(X_train)
    X_test_features = feature_extraction_func(X_test)
    ...
    visualize_lda_3d(X_train_lda, y_train, terrain_classes, 
                    title=f"3D LDA Projektion - {experiment_name}",
                    filename=f"lda_3d_{experiment_name.lower().replace(' ', '_')}.png")
    ...
    accuracy = evaluate_model(knn, X_test_lda, y_test, terrain_classes, 
                             filename_suffix=experiment_name.lower().replace(' ', '_'))
    
    print(f"Test nøjagtighed: {accuracy:.4f}")
    
    return accuracy, X_train_lda, X_test_lda

def main():
    """
    Hovedfunktion til at køre begge eksperimenter.
    """
    # Kør RGB + Tekstur eksperiment
    rgb_accuracy, X_train_rgb_lda, X_test_rgb_lda = run_experiment(
        "RGB_Texture", 
        extract_features_rgb_texture
    )
    
    # Kør HSV + Tekstur eksperiment
    hsv_accuracy, X_train_hsv_lda, X_test_hsv_lda = run_experiment(
        "HSV_Texture", 
        extract_features_hsv_texture
    )
    
    # Sammenlign resultater
    print("\n" + "="*50)
    print("SAMMENLIGNING AF EKSPERIMENTER")
    print("="*50)
    print(f"RGB + Tekstur nøjagtighed: {rgb_accuracy:.4f}")
    print(f"HSV + Tekstur nøjagtighed: {hsv_accuracy:.4f}")
    
    if rgb_accuracy > hsv_accuracy:
        print("RGB + Tekstur klarede sig bedre!")
    elif hsv_accuracy > rgb_accuracy:
        print("HSV + Tekstur klarede sig bedre!")
    else:
        print("Begge metoder klarede sig lige godt!")
